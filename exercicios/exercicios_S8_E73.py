...
def dados_pesquisa(sexo, cor_dos_olhos, idade):
    ...
    for loops in range(quant_pesquisado):
        ...
        chave_sexo = input('\nSexo (Feminino = F / Masculino = M / Outro = O):'
                           '\nOpção: ')
        chave_sexo = chave_sexo.upper()
        chave_cor_dos_olhos = input('Cor dos  olhos (Azuis = A / Verdes = V / Castanhos = C):'
                                    '\nOpção: ')
        chave_cor_dos_olhos = chave_cor_dos_olhos.upper()
        chave_idade = int(input('Idade: '))

        # Transformado os dados cadastrados em um dicionário
        dados = dict(sexo=chave_sexo, cor_dos_olhos=chave_cor_dos_olhos, idade=chave_idade)

        # Agregando o dicionário gerado na lista
        inf_pesquisa.append(dados)
